...
app.post("/webhook", async (req, res) => {
  ...
  const mediaType = message.type; // image, video, document, etc.

  try {
    await sendSeen(chatId);
  } catch (_) {}

  console.log(`📩 Mensaje de ${chatId} | tipo: ${mediaType} | hasMedia: ${hasMedia}`);
  ...
  if (!hasMedia || mediaType !== "image") {
    ...
    if (mediaType === "text") {
      ...
      if (body === "hola" || body === "start" || body === "help") {
        const reply =
  ...
  try {
    ...
    await startTyping(chatId);
    await wait(typingDelayMs(processingMsg.length));
    await stopTyping(chatId);
    await sendText(chatId, processingMsg);
  } catch (_) {}
  ...
  try {
    ...
    try {
      await sendText(
        chatId,
        "😅 Hubo un error al convertir la imagen. Intenta con otra imagen."
      );
    } catch (_) {}
  }
});
